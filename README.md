...
Nice Code uses a Rust engine for analysis and a Bun launcher for local checkout
usage. GitHub Releases will provide prebuilt engines; npm distribution is
...
```bash
bun scripts/init.mjs --project /path/to/project
```
...
When npm publication is enabled, the same Bun launcher will provide the
`nice-code` command without changing the Rust engine interface.
