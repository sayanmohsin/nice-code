...
export function runNativeTools(project: string) {
  const results: unknown[] = [];
  function addIfProject(manifest: string, command: string, args: string[]) {
  ...
  if (existsSync(join(project, "tsconfig.json"))) {
    ...
    results.push(
      existsSync(command)
        ...
        : {
            command: `${command} --noEmit`,
            status: "SKIPPED",
            output: "local TypeScript is not installed",
          },
    );
  }
    ...
    results.push(
      existsSync(command)
        ...
        : {
            ...
            status: "SKIPPED",
            output: "local Biome is not installed",
          },
    );
  }
